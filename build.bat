...
call hipcc -std=c++17 -O3 --offload-arch=gfx1102 -lrocblas tmp/*.o -o tmp/sgemm.exe

echo Building Kernel6 from ISA
call clang -target amdgcn-amd-amdhsa -mcpu=gfx1102 -c src/kernel6_valu_optim.s -o tmp/kernel6_device_code.o
call ld.lld -shared tmp/kernel6_device_code.o -o tmp/kernel6.hsaco

echo Building Kernel7 from ISA
call clang -target amdgcn-amd-amdhsa -mcpu=gfx1102 -c src/kernel7_unroll.s -o tmp/kernel7_device_code.o
call ld.lld -shared tmp/kernel7_device_code.o -o tmp/kernel7.hsaco

echo Building Kernel8 from ISA
call clang -target amdgcn-amd-amdhsa -mcpu=gfx1102 -c src/kernel8_batched_gmem.s -o tmp/kernel8_device_code.o
call ld.lld -shared tmp/kernel8_device_code.o -o tmp/kernel8.hsaco
